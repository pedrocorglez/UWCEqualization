...
data = randi([0 M-1],length(dataCarriers),1); %K random data 

% We modulate the data in QPSK symbols
...
symbol_t_cp = [symbol_t(K-CP+1:end) symbol_t];

figure(3)
plot(1:CP,real(symbol_t_cp(1:CP)),'r')
hold on
grid on
plot(CP:K+CP, real(symbol_t_cp(CP:K+CP)),'b')
...
% Channel convolution
symbol_t_cp_r = conv(symbol_t_cp, h);
symbol_t_cp_r = symbol_t_cp_r(1:K+CP);
% symbol_t_cp_r = awgn(symbol_t_cp_r, SNR);

% Remove CP
symbol_t_r = symbol_t_cp_r(CP+1:CP+K);
...
% Equalization
symbol_eq = symbol_r ./ H;
symbol_eq_2 = symbol_r ./ H;
